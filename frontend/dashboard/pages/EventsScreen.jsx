...
const EventsScreen = () => {
  ...
  const { isDark: darkMode, toggleTheme } = useTheme(); //isdark renamed as darkmode
...
//opens the day events popup when a day cell is clicked in calendar
  const openDayEvents = (day) => {
    ...
    const sorted = [...day.events].sort(
        (a, b) => new Date(a.start_time) - new Date(b.start_time)
    ); //sort by start time if positive a comes first else b comes first
    setSelectedDayInfo({
      date: day.date,
      events: sorted,
    });
